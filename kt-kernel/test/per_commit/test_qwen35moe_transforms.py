...
from kt_kernel.sglang_gguf_qwen35moe import (
    norm_sub_one,
    alog_transform,
    conv1d_reorder,
...
def test_apply_f32_transforms_conv1d():
    w = torch.zeros(4, 12, dtype=torch.float32)
    w[:, 4:] = torch.arange(4 * 8, dtype=torch.float32).reshape(4, 8)
    r = apply_f32_transforms(w, "blk.2.ssm_conv1d.weight")
    assert r.shape == (4, 1, 12)
    # V 段应被反序 (默认参数 1,1,1 意味着 qk_ch=1, v_ch=1)
    assert r[0, 0, 4] == w[3, 4]
